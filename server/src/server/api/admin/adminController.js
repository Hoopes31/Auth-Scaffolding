...
exports.findID = (req, res, next) => {
  Users.findOne({ username: req.query.username }).then(response => {
    if (response) {
...
exports.deleteUser = (req, res, next) => {
  Users.find({ _id: req.foundUserID })
    .remove()
    .exec()
...
exports.roleUpdate = (req, res, next) => {
  if (!req.query.roleUpdate || req.query.roleUpdate === "admin") {
    ...
    Users.findByIdAndUpdate(
      req.foundUserID,
      { role: req.query.roleUpdate },
      { runValidators: true }
    )
      .then(response =>
        res.send(
          `${response.username}'s role has been updated to ${req.query
            .roleUpdate}`
        )
      )
